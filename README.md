...
- **HRM (Hierarchical Reasoning Modal) support:** Enables tree‑structured reasoning with persistent scratch pad memory. Merged from the official HRM patch (Hugging Face) and fully integrated with MTP, NextN, and TurboQuant. Use `--hrm` and optionally `--scratch-system` to offload scratch to system RAM (essential for 8GB GPUs). See `src/models/hrm-text.h`.
...
- **This fork:** `--mmproj` can be loaded **alongside** `mtp` / `nextn` / `eagle3` / `hrm` speculative decoding on a single slot (validated on Qwen 3.6-35B-A3B-UDT + NextN + turbo3 KV and Gemma 4-26B-A4B + MTP + turbo3 KV + HRM). Draft acceleration applies to **text-only turns**; image-bearing turns fall back to plain target decoding (image still recognised). Other spec types remain disabled with multimodal. Details: [docs/speculative.md](./docs/speculative.md) and [NEXTN.md](./NEXTN.md) §10.
